...
- **Coverage on the research screens (SPEC 49.15, 2026-09-18)**: the same **Analysts** column now
  runs on `watchlist.html`, `screener.html` and the two screening-row lanes of `discovery.html`,
  and `index.html` carries a three-count strip built on the same partition. `DashboardService.
  putAnalyst` writes the fifteen keys onto the screening row map beside `putMacro`, which serves
  **screener and discovery in one change** because both read that map (B-099); `WatchlistItemView`
  gains the same fifteen as record components. One bulk query per screen. **A null `Coverage`
  writes nothing** - absent keys mean "did not look" and a counted 0 means "looked, found none",
  and collapsing those lets a failed query render as "no broker covers this stock" (SPEC 49.7).
  Measured before building (B-113): **213 of 274** screened stocks carry a live target, **23 of 26**
  on the contrarian lane, 9/6/8 across the three states on the watchlist. **Coverage runs opposite
  to our own score** - 61% at composite 80+, **84% below 50** - so a densely-covered stock is not a
  better one. The screener's column sits behind a display-only toggle purely for width (the
  20-column budget of SPEC 27.10); the row filter is always on. **Not** on discovery's insider,
  universe, IPO or buyability tables - those rows carry no `analyst*` fields, so the column would
  say "not measured" on every line (Gotcha 120), the same restriction the macro column has.
  **`analystCoverageLine` / `Panel` / `Strip` now share one `coveragePartition`** - they each
  re-derived the four groups before, which is exactly the shape of B-117 - and the line takes a
  **noun**, because it said "of N holdings" unconditionally and would have said that on the screener.
- **A target the price has already passed is not counted (SPEC 49.16, B-127)**: the measurement
  pass retires a target only in the direction the call was made - `direction` is
  `targetPrice >= priceAtCall` - so a target published at or just below the price of the day is
  filed as a **downward** call, and when the price runs **up** past it the call never resolves and
  sits in the live set dragging the median down. **Measured: 43 of 459 running targets, every one
  overtaken** (18 Sell, 21 Hold, 4 Buy); CPPLUS reported an upside of **-4.6%** to a median that
  sat below its own stored price. `coverage()` now partitions the live set against the stored price
  and counts the rest in `overtakenTargets`. Three rules travel with it: **the firm is not dropped**
  (a house whose target was overtaken still covers the stock - removing it could report a covered
  stock as uncovered, SPEC 49.7); **the absence is explained** ("price past its target" / "N passed"
  / a Passed column) because a median that quietly stops appearing is the shape of the bug rather
  than the fix (Gotcha 44, B-117); and **an absent stored price drops nothing**, since treating an
  unknown price as zero would call every target passed. Comparison is against the **stored** price,
  the same basis the upside is named in (SPEC 49.14), so a target the live price has passed is
  still counted until the 13:20 pass next measures that stock.
