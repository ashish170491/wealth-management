...
Write-Host "- Initial Capital: ₹50,000" -ForegroundColor Gray
...
try {
    ...
    if ($response.totalTrades -gt 0) {
        Write-Host "  Average Win: ₹$($response.averageWin)" -ForegroundColor Green
        Write-Host "  Average Loss: ₹$($response.averageLoss)" -ForegroundColor Red
    }
} catch {
