...
| B-013 | P2 | Stale-symbol replacements pending verification | 2026-05-10 | Removed 23 stale symbols (B-005). 2026-05-23: removed `GSPL` from the Nifty200 pool (Kite `/quote` returns `{status=success, data={}}` — invalid tradingsymbol) and made `KWIL-BE` resolve via NSE trading-series-suffix stripping in `KiteBrokerClient.getQuote` (`-BE/-BZ/-BL/-IL` → plain symbol). TATAMOTORS, LTIM still need verified post-corporate-action tradingsymbols before re-adding. Don't replace by guess — wrong symbol = wrong company analysed. **2026-09-19, four more, same signature.** The new `POST /api/multibagger/screen-unscored` pass (SPEC 12.13) reported the identical four symbols in its `failed` list on two consecutive runs: **NSE:HEG, NSE:HFCL, NSE:STLTECH, NSE:MTARTECH**, each failing at `fetchDailyHistory` with `{status=success, data={}}` from `/quote` while every other symbol in the same pass resolved — so not an out-of-hours or rate-limit artefact, and the request URL is identical in shape to the ones that work. All four are real, liquid NSE companies, so the likely cause is a tradingsymbol change (demerger, rename, re-listing) the app has not picked up; there is no instruments cache to look one up in since `KiteInstrumentsService` was deleted on 2026-09-03. **HEG is long-standing** — the same ERROR appears from 09:22 that morning and it sits in the `Nifty200WatchlistService` smallcap list, not in anything added recently. The other three were added by the theme pass (SPEC 51.4) and are all in `universe-sectors.csv`, which is how they passed the theme map's confirmation check — **that check asks whether NSE's index lists name the ticker, not whether the broker can price it**, and the gap between those two questions is the finding. *Blast radius*: 4 symbols × ~2 paced Kite calls × every screening run, for nothing (Gotcha 22's pattern). It distorts no score — a failed symbol simply gets no row — but the Themes coverage screen counts them as SCREENED because they are in the universe. *Not removed*: deleting them would make the Themes gap list say "add these" about stocks the app cannot reach, which is a worse claim than the cost. *Note what surfaced them*: these failed at `log.debug` inside the screening loop for as long as they have been in the universe; the new pass reporting its `failed` list as a first-class outcome is what made them visible — the argument SPEC 44 makes for the data-health screen. |
